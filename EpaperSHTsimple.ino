...
void AT_CMD_Task(void *pvParamerters)
{
  ...
  while (true)
  {
    if (xQueueReceive(receiveData_queue, (void *)atcmd, (portTickType)portMAX_DELAY))
    {
      ...
      if (param->temp_param.output_flag == 0 && getSensorData_handler != NULL && wifiConnect_handler != NULL)
      {
...
void getSensorData_Task(void *pvParamerters)
{
  ...
  while(true)
  {
    while(true)
    {
      ...
      if(param->cfg_param.work_mode == 1)
      show_info_text();
      ...
      if(!param->cfg_param.work_mode)
      {
        LOG("delay time 5s\r\n");
        show_info_wifi_mqtt_status(4);
        delay(5 * 1000);
      }
      else if(param->cfg_param.work_mode )
      {
        LOG("break\r\n");
        break;
      }
    ...
    if(param->cfg_param.work_mode == 2)
    {
      wifi_flag = 4;
      delay(5 * 1000);
    }
...
{
  do{
    while(wifi_flag != 2 && wifi_flag != 4)
    {
      delay(200);
    }
    if(wifi_flag == 4)
      wifi_flag = 0;
    sendTempAndHumi(&bm8563_date, param->cfg_param.device_id, temp, humidity);
  }while(wifi_flag == 0);
